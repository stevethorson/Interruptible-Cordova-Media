...
var Mediax = function(src, successCallback, errorCallback, statusCallback, beginInterruptionCallback, endInterruptionCallback) {
    ...
    window.mediaxObjects[this.id] = this;
    this.src = src;
    ...
    this.createdSuccess = function(){
        steroids.logger.log("just checking id: " + self.id);
        exec(function(result){steroids.logger.log("startListeningForAudioSessionEvent successful. Returned ID: " + result)}, function(err){steroids.logger.log("native layer error")}, "Media", "startListeningForAudioSessionEvent", [self.id, self.src]);
    }
...
Mediax.get = function(id) {
    return window.mediaxObjects[id];
};
...
Mediax.prototype.interruptionBegan = function(message, id) {
    ...
    var media = window.mediaxObjects[id];
